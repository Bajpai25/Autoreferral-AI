...
function parseResumeAnalysis(raw) {
    try {
...
const processResume = async (req, res) => {
    try {
        ...
        // --------------------------------------------------
        const existingResume = await constant_1.prisma.resume.findUnique({
        ...
        if (existingResume) {
            ...
            const textContent = await new Promise((resolve, reject) => {
                let rows = {};
                new pdfreader_1.PdfReader().parseBuffer(file.buffer, (err, item) => {
                    if (err)
                        reject(err);
                    else if (!item) {
            ...
            // 4 Save resume
            // --------------------------------------------------
            resumeRecord = await constant_1.prisma.resume.create({
                data: {
                    content: cleanedText,
                    userId,
        ...
        const response = await fetch("https://openrouter.ai/api/v1/chat/completions", {
            ...
            body: JSON.stringify({
                ...
                messages: [
                    {
                        role: "system",
                        content: message_1.resumeSummariser
                    },
                    {
                        role: "user",
        ...
        // 6 Update summary
        // --------------------------------------------------
        const updatedResume = await constant_1.prisma.resume.update({
